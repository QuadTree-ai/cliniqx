...
import { getDb, ensureReportsCollection } from '../../lib/mongodb'; // Adjust path as needed
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    await ensureReportsCollection(); // Ensure the 'reports' collection exists
    ...
    console.log('Inserting into MongoDB:', reportData);

    // Insert report data into MongoDB
    const result = await db.collection('reports').insertOne(reportData);

    if (result.acknowledged) {
      console.log('Report successfully inserted:', result.insertedId);
    } else {
