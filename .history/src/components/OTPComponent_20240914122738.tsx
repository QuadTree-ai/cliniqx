...
import { OTP_LENGTH, INITIAL_TIMER, RESEND_OTP_MESSAGE, PHONE_OTP_HEADER, PHONE_OTP_DESCRIPTION } from "@/app/otp/otpconst";
...
const OTPComponent: React.FC<OTPComponentProps> = ({ phoneNumber, onOTPVerified, onClose }) => {
  ...
  const verifyOTP = () => {
    setLoading(true);
    const isValidOTP = otp.join("") === actualOTP;
    if (isValidOTP) {
      onOTPVerified();
    } else {
