...
const NAV_ITEMS = [
  ...
  { href: "/dashboard/settings", icon: <SettingsIcon className="h-5 w-5" />, label: "Settings", className: "mt-auto" }
];
          ...
          {NAV_ITEMS.map((item, index) => (
            <Tooltip key={index}>
              <TooltipTrigger asChild>
                <Link
                  href={item.href}
                  className={`flex h-9 w-9 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:text-foreground md:h-8 md:w-8 ${item.className || ""}`}
                  prefetch={false}
                >
                  {item.icon}
