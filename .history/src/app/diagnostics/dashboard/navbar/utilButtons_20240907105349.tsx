...
const TopButtons: React.FC<TopButtonsProps> = ({ setActiveSection }) => (
  <div className="flex flex-wrap justify-center gap-4 p-6 bg-white shadow-md">
    {buttonConfig.map(({ icon: Icon, label, section }) => (
      <Button
        key={section}
        onClick={() => setActiveSection(section)}
        variant="ghost"
        className="flex-1 min-w-[150px] h-16 bg-white text-gray-700 hover:bg-gray-100 hover:text-gray-900 transition-colors duration-200 ease-in-out rounded-lg shadow-sm border border-gray-200"
      >
