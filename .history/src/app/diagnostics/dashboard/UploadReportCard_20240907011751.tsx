  ...
  return (
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      ...
      <Card className="col-span-1 lg:col-span-2 relative">
        {files.length > 0 && (
          <Button onClick={handleShareReport} className="absolute top-4 left-4 py-1 text-sm flex items-center justify-center">
            ...
            <div className="mt-4 w-full">
              <div className="flex flex-wrap gap-4">
                {previews.map((preview, index) => (
                  preview && (
                    <img key={index} src={preview} alt={files[index].name} className="w-24 h-24 object-cover rounded" />
                  )
                ))}
              </div>
              {uploadProgress < 100 && (
