    ...
    <div className="snap-y snap-mandatory h-screen overflow-scroll" style={{ scrollBehavior: 'smooth' }}>
      ...
      {signupOptions.map((option) => (
        <div key={option.id} className="h-screen flex items-center justify-center snap-start">
          <HoverCard className="container mx-auto text-center p-6 transition-all duration-300 bg-white rounded-lg shadow-xl hover:shadow-2xl">
            ...
            <Tooltip content={option.detail}>
              <p className="text-sm text-gray-500 mb-2">{option.description}</p>
            </Tooltip>
            <p className="text-sm text-gray-500 mb-4">{option.detail}</p>
            <Link href={option.link} passHref>
              <Button className={`rounded-md py-2 px-4 text-white ${option.buttonClass} transition-colors duration-200`}>
                {option.title}
              </Button>
            </Link>
          </HoverCard>
