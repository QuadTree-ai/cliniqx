...
import { formFields, FormFieldKey } from "./FormConstants"; // Adjust the path as necessary

function FormField({ fieldKey, value, handleInputChange }) {
...
export default function CustomerSignup() {
    ...
    const handleInputChange = (e) => {
        ...
        if (['first-name', 'last-name'].includes(name)) {
            setShowBSCard(value.trim() !== "");
        }
    };
                ...
                <CardContent>
                    <form onSubmit={handleSubmit}>
                        <div className="grid gap-4">
                            {['firstName', 'lastName', 'email', 'phone', 'password'].map(key => (
                                <FormField key={key} fieldKey={key} value={formData[key]} handleInputChange={handleInputChange} />
                            ))}
