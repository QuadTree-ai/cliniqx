...
import { Button, Card, CardContent, CardHeader, Checkbox, Input, Label } from "@/components/ui";
...
import { formFields, FormFieldKey } from "./FormConstants";
...
const FormField: React.FC<FormFieldProps> = ({ fieldKey, value, handleInputChange }) => {
...
export default function CustomerSignup() {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (name in formData) { // Ensuring that name is a valid key of formData
      setFormData(prev => ({ ...prev, [name]: value }));
      setShowBSCard((name === 'firstName' || name === 'lastName') && value.trim() !== "");
    }
  };
      ...
      <Card className="w-full max-w-lg p-6 shadow-xl rounded-xl bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg border border-gray-700">
        <CardHeader className="flex flex-col items-center">
          {!showBSCard ? (
            ...
            <BSCliniQXCard identifier={cardNumber} setIdentifier={setCardNumber} className="w-96" firstName={formData.firstName} lastName={formData.lastName} />
          )}
        </CardHeader>
        <CardContent>
            ...
            <div className="grid gap-4">
              {["firstName", "lastName", "email", "phone", "password"].map(
                (key) => (
                  <FormField
                    key={key}
                    fieldKey={key as FormFieldKey}  // Ensuring correct type casting
                    value={formData[key]}
