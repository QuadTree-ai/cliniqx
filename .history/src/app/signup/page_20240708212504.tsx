...
export default function MainSignup() {
  return (
    <div className="snap-y snap-mandatory h-screen overflow-y-scroll" style={{ scrollBehavior: 'smooth' }}>
      ...
      <div className="fixed top-1/2 transform -translate-y-1/2 left-4 z-10 flex flex-col space-y-4">
        {signupOptions.map((option, index) => (
          <a key={option.id} href={`#section-${index}`} className="w-4 h-4 rounded-full bg-gray-300 hover:bg-blue-500 transition-colors duration-200"></a>
        ))}
      </div>
      ...
      {signupOptions.map((option, index) => (
        <div key={option.id} id={`section-${index}`} className="snap-start flex items-center justify-center h-screen bg-gray-100">
          <Card className="container mx-auto text-center p-6 transition-all duration-300 bg-white rounded-lg shadow-lg flex flex-col items-center">
            <img src={option.imgSrc} alt={option.title} className="w-full h-64 object-cover rounded-md mb-4 shadow-md" />
            <h2 className="text-4xl font-bold text-gray-800 mb-4">{option.title}</h2>
