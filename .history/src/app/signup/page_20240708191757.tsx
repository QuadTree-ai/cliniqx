...
export default function MainSignup() {
  ...
  const handleSignup = (optionId, title) => {
    toast({
    ...
    <div className="snap-y snap-mandatory h-screen overflow-scroll" style={{ scrollBehavior: 'smooth' }}>
      ...
      {signupOptions.map((option) => (
        <div key={option.id} className="h-screen flex items-center justify-center snap-start">
          <HoverCard className="container mx-auto text-center p-6 transition-all duration-300 bg-white rounded-lg shadow-lg hover:shadow-xl">
            <img src={option.imgSrc} alt={`${option.title} Image`} className="w-32 h-32 rounded-full mb-4"/>
            {option.icon}
            <Tooltip label={option.detail} hasArrow>
            ...
            <Link href={option.link} passHref>
              <Button onClick={() => handleSignup(option.id, option.title)} className={`rounded-md py-2 px-4 text-white ${option.buttonClass} transition-colors duration-200`}>
                {option.title}
              </Button>
            </Link>
          </HoverCard>
