          ...
          {signupOptions.map((option) => (
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-black via-transparent to-transparent">
                ...
                <p className="text-base text-black mb-4">{option.description}</p>
                <Link href={option.link} passHref>
                  <Button className="rounded-md py-3 px-8 text-black bg-yellow-600 hover:bg-yellow-700 transition-colors duration-200 flex items-center space-x-2">
                    <option.Icon className="w-6 h-6 text-black" />
                    <span>{option.signupText}</span>
                  </Button>
                </Link>
              </div>
              <div className="absolute inset-0 bg-gradient-to-b from-gray-100 via-gray-200 to-gray-300 opacity-50 pointer-events-none"></div>
            </Card>
          ))}
        </div>
      </section>
