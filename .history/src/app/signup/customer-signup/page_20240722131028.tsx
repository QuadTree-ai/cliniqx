...
export default function CustomerSignup() {
  ...
  const [cardNumber] = useState("00001"); // Maintain a constant card number for the session
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-700 relative">
      ...
      <Card className="w-full max-w-lg p-6 shadow-lg backdrop-blur-lg z-10">
        <CardHeader className="flex flex-col items-center">
          {!showBSCard ? (
            // Display the initial CliniQXCard with the default card number
            <CliniQXCard identifier={cardNumber} setIdentifier={() => {}} className="w-96" />
          ) : (
            // Display the BSCliniQXCard without changing the card number
            <BSCliniQXCard identifier={cardNumber} className="w-96" />
          )}
        </CardHeader>
        <CardContent>
