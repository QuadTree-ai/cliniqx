...
export default function CustomerSignup() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Include identifier in form data submission
    const formData = new FormData(e.target as HTMLFormElement);
    formData.append('identifier', identifier);
    // Perform form submission here (e.g., send to server)
    setShowOTP(true);
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-700 relative backdrop-blur-sm">
      <Background />
      <Card className="w-full max-w-lg p-6 shadow-lg backdrop-blur-lg z-10">
        <form onSubmit={handleSubmit}>
          <CardHeader className="flex flex-col items-center">
            <CliniQXCard identifier={identifier} setIdentifier={setIdentifier} className='mb-4'/>
          </CardHeader>
          <CardContent>
