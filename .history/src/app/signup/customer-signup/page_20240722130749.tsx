...
export default function CustomerSignup() {
  ...
  const [cardNumber] = useState("00001"); // cardNumber is now a constant within this component instance
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-700 relative">
      ...
      <Card className="w-full max-w-lg p-6 shadow-lg backdrop-blur-lg z-10">
        <CardHeader className="flex flex-col items-center">
          {!showBSCard ? (
            <CliniQXCard identifier={cardNumber} setIdentifier={() => {}} className="w-96" /> // Prevent changing the card number here
          ) : (
            <BSCliniQXCard identifier={cardNumber} className="w-96" /> // Do not pass setIdentifier to maintain constant card number
          )}
        </CardHeader>
        <CardContent>
