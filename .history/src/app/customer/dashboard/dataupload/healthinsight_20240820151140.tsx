...
import HumanAnatomyIcons, { iconsMap } from '../icon/anatomyicons'; // Import the HumanAnatomyIcons component
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {insights.map(({ part, problem, severity }, index) => (
        <Card key={index} className={`border ${getSeverityColor(severity)} shadow-lg`}>
          <CardHeader className="flex flex-col items-center">
            <HumanAnatomyIcons icon={part} size={48} color={getSeverityIconColor(severity)} />
            <CardTitle className="text-xl font-semibold capitalize mt-4">{part}</CardTitle>
          </CardHeader>
          <CardContent className="text-center">
            <p className="text-gray-700">{problem}</p>
            <Badge variant="outline" className={`mt-4 ${getSeverityBgColor(severity)}`}>
