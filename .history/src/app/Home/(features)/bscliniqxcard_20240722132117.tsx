...
const BSCliniQXCard: React.FC<BSCliniQXCardProps> = ({ identifier = "00001", setIdentifier, firstName, lastName, className }) => {
  useEffect(() => {
    if (setIdentifier) {
    ...
    <div className={`flex flex-col items-center space-y-8 ${className}`}>
      <Card className="w-96 h-56 bg-black text-white rounded-2xl shadow-lg relative p-4 flex flex-col justify-center items-center">
        <div className="absolute top-0 left-0 h-full flex items-center justify-start overflow-hidden">
          <span className="font-baumans text-[14rem] leading-none text-gray-300 translate-x-[-2.8rem] translate-y-[-1rem]">X</span>
        </div>
        ...
        <div className="absolute bottom-4 right-4 text-right text-xs leading-none flex flex-col items-end">
          <div className="font-lovers-quarrel text-sm -translate-y-1">by</div>
