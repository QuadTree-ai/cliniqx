...
const CliniQXCard: React.FC<CliniQXCardProps> = ({ identifier = "00001", setIdentifier, className, ...props }) => {
  ...
  return (
    <div className={`flex flex-col items-center space-y-8 ${className}`} {...props}>
      <Card className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl h-56 bg-black text-white rounded-2xl shadow-lg relative p-4 flex flex-col justify-center items-center">
        <div className="flex items-center justify-center">
          <span className="font-capriola text-lg sm:text-xl md:text-2xl font-medium">cliniQ</span>
          <span className="font-baumans text-2xl sm:text-3xl md:text-4xl">X</span>
        </div>
