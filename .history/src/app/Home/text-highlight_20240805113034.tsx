...
const TextHighlight: React.FC = () => {
  ...
  const handleScroll = () => {
    const triggerHeight = window.innerHeight / 3; // Lowering the trigger point to react sooner
  ...
  const getLineVariants = (delay: number) => ({
    ...
    visible: { opacity: 1, color: "#FFFFFF", scale: 1.05, transition: { delay, duration: 0.3 } }, // Shortened duration
  });

  return (
    <div className="min-h-screen flex items-center justify-center bg-black" style={{ minHeight: '150vh' }}> {/* Increased section length */}
      <div className="text-center font-bold text-8xl leading-tight space-y-8 text-gray-400">
        <motion.span
          ref={line1Ref}
          className="block"
          variants={getLineVariants(0.1)} // Reduced initial delay
          initial="hidden"
        ...
        <motion.span
          ref={line2Ref}
          className="block"
          variants={getLineVariants(0.1)} // Consistent reduced delay
          initial="hidden"
