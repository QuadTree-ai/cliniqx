...
const Statistics = () => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-r from-black via-gray-900 to-black p-8">
      <div>
        <h1 className="text-5xl font-bold text-white mb-10 text-center">Today&apos;s Healthcare Impact</h1>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 text-center">
          {statistics.map((stat, index) => (
            <motion.div key={index} className="py-6" initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: index * 0.1 } }}>
              <stat.icon className="mx-auto text-cyan-400 text-7xl mb-3" />
              <h2 className="text-2xl font-semibold text-white">{stat.name}</h2>
