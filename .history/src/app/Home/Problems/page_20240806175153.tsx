...
export function CardsContainer() {
  ...
  useEffect(() => {
    ...
    setStatistics([...updatedStats]); // Use original array for better duplication handling
  }, []);

  return (
    <section className="bg-gray-100 flex flex-col justify-center items-center h-screen w-full overflow-hidden">
      <div className="text-center mb-12">
        <h1 className="text-5xl font-bold text-black mb-6">Health Impact Statistics</h1>
        <Badge variant="outline" className="mt-4">Health-related Incidents</Badge>
      </div>
      <div className="marquee w-full">
        <div className="marquee-content flex animate-marquee">
          {statistics.concat(statistics).map((stat, index) => (
            <CardDemo
              key={index}
              title={stat.name}
