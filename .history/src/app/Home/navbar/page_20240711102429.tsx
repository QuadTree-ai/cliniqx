    ...
    <nav className="fixed w-full z-20 top-0 bg-white">
      ...
      {isOpen && (
        <div className="md:hidden flex flex-col items-center bg-gray-50 py-4 space-y-4">
          <NavMenu />
          <Link href="/login" passHref>
            <Button className="text-white bg-yellow-500 hover:bg-yellow-600 transition-colors duration-200 text-md w-full text-center">
              Login
            </Button>
          </Link>
          <Link href="/signup/customer-signup" passHref>
            <Button className="text-white bg-black hover:bg-gray-800 transition-colors duration-200 flex items-center space-x-2 text-md w-full text-center">
              <span>Try Now</span>
              <ArrowRight className="w-5 h-5" />
            </Button>
          </Link>
