      ...
      <NavigationMenuList className="flex flex-wrap justify-between md:justify-start space-x-0 md:space-x-4">
        {navMenuItems.map((item, index) => (
          <NavigationMenuItem key={index} className="w-full md:w-auto">
            <NavigationMenuTrigger className="flex items-center space-x-2 px-4 py-2 rounded-lg w-full md:w-auto text-lg">
              <span>{item.label}</span>
            </NavigationMenuTrigger>
            <NavigationMenuContent>
              <ul className="grid gap-3 p-6 w-full md:w-[600px] lg:w-[700px] lg:grid-cols-[.75fr_1fr]">
