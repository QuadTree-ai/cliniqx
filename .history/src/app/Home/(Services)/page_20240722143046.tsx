...
import { MagnifyingGlass } from "lucide-react"; // For indicating zoomable images

const Service = () => {
  // Function to handle image click, could open a modal or lightbox
  const handleImageClick = (imageSrc) => {
    // This function could potentially open a modal or trigger a lightbox view
                    ...
                    <div className="flex flex-wrap justify-start items-center gap-4">
                      {service.subServices.map((subService, subIndex) => (
                        <div key={subIndex} className="bg-gray-50 p-4 rounded-lg shadow-sm hover:scale-105 transition-transform duration-300 ease-in-out w-full">
                          <div className="relative w-full h-48 mb-2 cursor-pointer" onClick={() => handleImageClick(subService.image)}>
                            <Image
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                              <MagnifyingGlass className="text-white w-8 h-8" />
                            </div>
                          </div>
