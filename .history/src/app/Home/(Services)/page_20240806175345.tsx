...
const ServiceCardComponent = ({ card, index }: { card: ServiceCard, index: number }) => (
  <Card
    card={{
      ...
      category: "Healthcare",
      content: (
        <div className="p-4 bg-white bg-opacity-80 backdrop-filter backdrop-blur-md rounded-lg shadow-lg h-80">
          <h2 className="text-2xl font-semibold text-gray-800 mb-2">{card.title}</h2>
          <p className="text-md mb-4">{card.description}</p>
          <div className="space-y-2">
            {card.subServices.map((subService, subIndex) => (
              <div key={subIndex} className="flex items-center space-x-2">
                <p className="text-sm text-gray-700">{subService.text}</p>
              </div>
            ))}
          </div>
        </div>
...
const Service: React.FC = () => {
  ...
  return (
    <section className="text-gray-800 py-16 dark:bg-gray-900">
      <div className="container mx-auto px-6">
        <h2 className="max-w-7xl mx-auto text-center text-3xl md:text-5xl font-bold text-white dark:text-white mb-4">
          Our Services
        </h2>
