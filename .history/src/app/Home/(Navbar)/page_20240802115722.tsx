    ...
    <nav className="w-full z-50 top-0 backdrop-filter backdrop-blur-lg relative">
      ...
      {isOpen && (
        <div className="flex flex-col items-center bg-gray-50 py-4 space-y-4">
          <NavMenu />
          <Link href="/login/customer" passHref>
            <Button className="text-white bg-[#E2C044] hover:bg-[#cca03a] transition-colors duration-200 text-base w-full text-center">
              Login
            </Button>
          </Link>
          <Link href="/signup/customer" passHref>
            <Button className="text-white bg-[#136D94] hover:bg-[#0f5b7a] transition-colors duration-200 flex items-center space-x-2 text-base w-full text-center">
              <span>Try Now</span>
              <ArrowRight className="w-5 h-5" />
            </Button>
          </Link>
