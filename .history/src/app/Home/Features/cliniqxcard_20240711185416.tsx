...
const CliniQXCard = () => {
  ...
  useEffect(() => {
    const generateAlphanumeric = () => {
      // Generate UUID, remove non-alphanumeric characters, and convert to uppercase
      const alphanumeric = uuidv4().replace(/[^a-zA-Z0-9]/g, '').toUpperCase();
      // Filter out non-uppercase characters and slice the first 8 characters
      return alphanumeric.replace(/[^A-Z]/g, '').slice(0, 8);
    };
