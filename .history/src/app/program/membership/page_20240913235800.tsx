      ...
      <div className="w-full max-w-6xl space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {plans.map((plan, index) => {
            ...
            return (
              <Card key={index} className="w-full bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg border border-gray-700 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                <CardHeader className="flex items-center space-x-4">
                  <Icon className="w-10 h-10 text-blue-500" />
                  <div>
                    <CardTitle className="text-lg font-semibold text-white">{plan.title}</CardTitle>
                    <CardDescription className="text-2xl font-bold text-white mt-2">{plan.price}</CardDescription>
                  </div>
                </CardHeader>
                <CardContent>
                    ...
                    {plan.benefits.map((benefit, benefitIndex) => {
                      ...
                      return (
                        <div key={benefitIndex} className="flex items-start space-x-2">
                          <BenefitIcon className="w-8 h-8 text-blue-500" />
                          <div>
