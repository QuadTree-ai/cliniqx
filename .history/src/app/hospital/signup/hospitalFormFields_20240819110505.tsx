...
const HospitalFormFields: React.FC<HospitalFormFieldsProps> = ({ signupData, handleChange }) => {
  ...
  const renderField = (field: FieldConfig) => {
    const value = signupData[field.name];
      ...
      <Card className="w-full max-w-lg p-6 shadow-xl rounded-xl bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg border border-gray-700">
        ...
        <CardContent>
          <form className="space-y-6">
            ...
            <div {...getRootProps()} className={`border-2 border-dashed p-6 rounded-md transition ${isDragActive ? "bg-gray-800 border-blue-500" : "bg-transparent border-gray-500"}`}>
              <input {...getInputProps()} />
              {isDragActive ? (
                <p className="text-white">Drop the files here ...</p>
              ) : (
                <p className="text-white">Drag and drop important files here, or click to select files</p>
              )}
