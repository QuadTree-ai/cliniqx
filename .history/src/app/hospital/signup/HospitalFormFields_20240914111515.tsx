      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {currentStepConfig.fields.map((field: DynamicFieldConfig) => (
          <div key={field.name} className="form-field mb-4">
            ...
            {renderField(field)}
            {errors[field.name.split('.')[0] as keyof HospitalSignupData] && (
              <span className="text-red-500 text-sm">
                {(errors[field.name.split('.')[0] as keyof HospitalSignupData] as any)?.[field.name.split('.')[1]]?.message ||
                 errors[field.name.split('.')[0] as keyof HospitalSignupData]?.message}
              </span>
            )}
          </div>
        ))}
