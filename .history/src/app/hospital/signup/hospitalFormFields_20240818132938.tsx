...
const fieldConfigurations: { [key: string]: FieldConfiguration[] } = {
  general: [
...
const renderInputField = (field: FieldConfiguration, value: string | number, handleChange: (e: React.ChangeEvent<HTMLInputElement>) => void) => (
...
const renderSelectField = (field: FieldConfiguration, value: string, options: string[], handleChange: (e: any) => void) => (
  <div key={field.name} className="mb-4">
    <Label htmlFor={field.name} className="block text-sm font-medium text-gray-700 mb-1">
      {field.label}
    </Label>
...
export const HospitalFormFields: React.FC<HospitalFormFieldsProps> = ({ signupData, handleChange }) => {
  return (
    <div>
      {/* General Information */}
      ...
      {fieldConfigurations.contact.map((field) =>
        renderInputField(field, signupData.contact[field.name], (e) => handleChange(e, "contact"))
      )}

      {/* Address Information */}
      <h3 className="text-lg font-semibold mt-6 mb-4">Address Information</h3>
      {fieldConfigurations.address.map((field) =>
        renderInputField(field, signupData.address[field.name], (e) => handleChange(e, "address"))
      )}

      {/* Staff Capacity */}
      <h3 className="text-lg font-semibold mt-6 mb-4">Staff Capacity</h3>
      {fieldConfigurations.staffCapacity.map((field) =>
        renderInputField(field, signupData.staffCapacity[field.name], (e) => handleChange(e, "staffCapacity"))
      )}
